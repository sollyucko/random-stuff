...
def apify_class(cls: Type[Managed]) -> RecursiveDefaultDict[str, wsgi.Callable]:
    ...
    def add_route(path: Iterable[str], methods: List[str]):
        ...
        def decorator(f: wsgi.Callable) -> wsgi.Callable:
            ...
            def g(
                environ: wsgi.Environ, start_response: wsgi.StartResponse
            ) -> wsgi.Iterable:
                if environ["REQUEST_METHOD"].upper() in methods:
                    ...
                    for expected, actual in zip_longest(
                        remainder, iter(partial(shift_path_info, environ), "")
                    ):
    ...
    def add_instance(
        ...
        try:
            instance = cls(
                **dict(line.split("\t", 1) for line in environ["wsgi.input"])
            )
        except TypeError:
    ...
    def get_instance_info(
        identifier: str, environ: wsgi.Environ, start_response: wsgi.StartResponse
    ) -> wsgi.Iterable:
...
class RecursiveMapping(
    Generic[KT, VT], Mapping[KT, Union[VT, "RecursiveMapping[KT, VT]"]], ABC
):
    pass


def call_dict(
    routes: Union[wsgi.Callable, RecursiveMapping[str, wsgi.Callable]],
    environ: wsgi.Environ,
    start_response: wsgi.StartResponse,
) -> wsgi.Iterable:
    while True:
...
class Application(wsgi.Callable):
    ...
    def __call__(
        self, environ: wsgi.Environ, start_response: wsgi.StartResponse
    ) -> wsgi.Iterable:
