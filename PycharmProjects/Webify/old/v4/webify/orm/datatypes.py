...
from webify.orm import DBValue, T, render_html_input, render_html_input_valued


def create_hasher(name: str) -> Callable[[str], str]:
    def hasher(data: str, *args: Any, **kwargs: Any) -> str:
        return hashlib.new(name, data.encode('UTF-8')).hexdigest(*args, **kwargs)  # type: ignore
...
class Password(DBValue):
    ...
    def __eq__(self, other: Any) -> Union[bool, Type[NotImplemented]]:  # type: ignore
    ...
    def from_value(cls: Type[Password], value: Union[str, Password], hasher_name: Optional[str] = None,
        ...
        if isinstance(value, Password):
            ...
            if hasher_name is None:
                raise TypeError('If value is a str, hasher_name must not be None.')
            
            return cls(cls.hashers[hasher_name](value, **kwargs), hasher_name)  # type: ignore
        else:
    ...
    @classmethod
    def get_db_type(cls: Type[Password]) -> Tuple[Type[str], Mapping[str, Any]]:
        return str, {}
    
    def to_db_value(self: Password) -> str:
