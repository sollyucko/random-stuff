...
class Tileset(object):
    ...
    def add_image(self, file):
        ...
        if not image:
            raise FileNotFoundError("Error creating new Tileset: file {} not found".format(file))
        id_ = self.firstgid
...
class Cell(object):
    ...
    def __repr__(self):
        return '<Cell {},{} {:d}>'.format(self.px, self.py, self.tile.gid)
...
class Layer(BaseLayer):
    ...
    def __repr__(self):
        return '<Layer "{}" at 0x{:x}>'.format(self.name, id(self))
    ...
    def fromxml(cls, tag, map_):
        ...
        if data is None:
            raise ValueError('layer {} does not contain <data>'.format(layer.name))
        ...
        if len(data):
            ...
            data = struct.unpack('<{:d}i'.format(len(data) // 4, ), data)
...
visible: Whether the object is shown (1) or hidden (0). Defaults to 1.
    ...
    def __repr__(self):
        if self.tile:
            return '<Object {},{} {},{} tile={:d}>'.format(self.px, self.py, self.width, self.height, self.gid)
        else:
            return '<Object {},{} {},{}>'.format(self.px, self.py, self.width, self.height)
...
class ObjectLayer(BaseLayer):
    ...
    def __repr__(self):
        return '<ObjectLayer "{}" at 0x{:x}>'.format(self.name, id(self))
