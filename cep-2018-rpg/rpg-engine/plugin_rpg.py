    ...
    def update(self, dt):
        ...
        for area in self.window.tilemap.layers['triggers'].collide(self.rect, 'probEnemy'):
            if old != self.rect and random() < float(area['probEnemy']):
                ...
                self.rect = old
                self.window.spawn(Enemy(
                    image=self.window.enemy_icon,
                    location=enemy_rect.topleft,
                    window=self.window,
        ...
        if entries:
            ...
            self.window.set_tilemap(
                map_path=entry['destinationMap'],
                ...
                self.rect = self.move(
                    max(
                        (
                            ...
                            if not self.window.collide_objects(self.move(direction))
                               and not self.window.collide_border(self.move(direction))
                               and not self.window.collide_sprite(self.move(direction))
                        ),
                        key=itemgetter(1)
                    )[0]
                )
